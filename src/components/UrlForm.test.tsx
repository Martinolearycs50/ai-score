...
expect(screen.getByRole('button', {
 name: /Analyze/i })).toBeInTheDocument();
});
test('should call onSubmit with valid URL',
  async () => {
...
test('should show error for invalid URL',
  async () => {
const user = userEvent.setup();
render(<UrlForm;
onSubmit={
...
const button = screen.getByRole('button', {
 name: /Analyze/i });
// Button should be disabled when input is empty expect(button).toBeDisabled();
expect(mockOnSubmit).not.toHaveBeenCalled();
});
...
test('should clear error when typing',
  async () => {
const user = userEvent.setup();
render(<UrlForm;
onSubmit={
...
const button = screen.getByRole('button', {
 name: /Analyze/i });
// First submit with invalid URL await user.type(input,
'invalid');
await user.click(button);
  await waitFor(() => {
expect(screen.getByText(/Invalid domain format|domain extension/i)).toBeInTheDocument();
});
// Clear and type valid URL await user.clear(input);
await user.type(input,
'https://example.com');
// Error should be cleared expect(screen.queryByText(/Please enter a valid URL/i)).not.toBeInTheDocument();
});
