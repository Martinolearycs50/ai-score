...
return;
 ...
 // Basic email validation const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
...
return;
 ...
 setIsSubmitting(true);
  try {
  // Call parent handler or default implementation if (onSubmit) {
await onSubmit(email);
};
 else {
// Default: simulate API call await new Promise(resolve => setTimeout(resolve, 1000));
console.log('Email submitted:',
email);
};
 setSubmitted(true);
setEmail('');
  // Reset after 5 seconds setTimeout(() => {
setSubmitted(false);
}, 5000);
};
 catch (err) {
...
strokeWidth="2"
strokeLinecap="round"
strokeLinejoin="round"/> </svg> <p;
className="text-lg;
font-medium;
success-text">Success!</p> <p;
...
>Check your email for AI optimization tips</p> </motion.div> );
};
 const variants = {
  inline: {
...
> {
styles.title}
 </h3> <p className="text-muted text-sm"> {
styles.subtitle}
 </p> <form;
onSubmit={
handleSubmit};
...
` ${
styles.inputClass};
 px-4 py-3 border border-default rounded-lg focus:outline-none focus:ring-2 focus:ring-accent disabled:opacity-50 `};
 style={
{
...
error && ( <p className="error-text text-sm mt-2">{
error}
</p> )}
 <p className="text-xs text-muted mt-3"> No spam,
unsubscribe anytime. By subscribing,
you agree to our privacy policy. </p> </motion.div> );
}
