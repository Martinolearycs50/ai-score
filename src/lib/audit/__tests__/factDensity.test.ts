...
describe('Fact Density Audit Module', () => {
  ...
  describe('Unique stats extraction', () => {
    it('should detect percentages', async () => {
      const html =
        '<html><body><p>Growth increased by 50% and efficiency by 75.5%</p></body></html>';
    ...
    it('should detect monetary values', async () => {
      const html =
        '<html><body><p>Revenue reached $5.2 million, up from $3M last year</p></body></html>';
      const scores = await run(html);
    ...
    it('should detect measurements', async () => {
      const html =
        '<html><body><p>The server processes 1000 requests per second at 50ms latency</p></body></html>';
      const scores = await run(html);
    ...
    it('should detect proper names and companies', async () => {
      const html =
        '<html><body><p>John Smith from Microsoft announced OpenAI partnership</p></body></html>';
      const scores = await run(html);
    ...
    it('should allow less than 10% duplication', async () => {
      const html = `
        <html><body>
          ${Array(20)
            .fill(0)
            .map((_, i) => `<p>Unique paragraph ${i} with different content and information.</p>`)
            .join('')}
  ...
  describe('Edge cases', () => {
    ...
    it('should handle content with no main area', async () => {
      const html =
        '<html><body><div>Some content with facts: 50% growth in 2024</div></body></html>';
      const scores = await run(html);
