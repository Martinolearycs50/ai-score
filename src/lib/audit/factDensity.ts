...
export async function run(html: string): Promise<FactDensityScores> {
  ...
  const contentText =
    $('main, article, [role="main"], .content, #content').text() || $('body').text();
...
function checkForDirectAnswer(heading: string, content: string): boolean {
  ...
  const hasEarlyVerb =
    /^(\w+\s+){0,3}(is|are|was|were|has|have|can|will|does|provides|offers|includes|helps|makes|allows|enables)\b/i.test(
      firstSentence
    );
...
function extractUniqueStats(text: string): Set<string> {
  ...
  const numbers =
    text.match(/[$€£¥]\d+[kKmMbB]?|\d+[,.]?\d*\s*(million|billion|thousand|hundred|x|X)/g) || [];
  numbers.forEach((n) => stats.add(n.trim()));

  // Extract specific measurements
  const measurements =
    text.match(
      /\d+\s*(mph|km\/h|kg|lbs|meters|feet|miles|kilometers|GB|MB|TB|ms|seconds?|minutes?|hours?|requests?\s*(?:per|\/)\s*(?:second|minute|hour)|req\/s|rps|qps)/gi
    ) || [];
...
function extractProperNames(text: string): Set<string> {
  ...
  for (let i = 0; i < words.length - 1; i++) {
    ...
    if (
      current.length > 4 &&
      /^[A-Z]/.test(current) &&
      (current.includes('Corp') ||
        current.includes('Inc') ||
        current.includes('LLC') ||
        current.includes('Ltd') ||
        /^[A-Z]+$/.test(current))
    ) {
      names.add(current);
    }
