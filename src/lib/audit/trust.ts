...
interface TrustScores {
  ...
  httpsUsage: number; // HTTPS protocol usage (+5) - NEW
}

/**
 * Audit module for the Trust pillar (20 points max) - UPDATED
 * Checks: Author bio, NAP consistency, content license, HTTPS usage
 */
export async function run(html: string, url?: string): Promise<TrustScores> {
  ...
  // NEW: Check for HTTPS usage
  if (url) {
    try {
      const urlObj = new URL(url);
      scores.httpsUsage = urlObj.protocol === 'https:' ? 5 : 0;
    } catch (e) {
      // Invalid URL, check canonical link
    ...
    // No URL provided, check canonical link
    const canonicalUrl = $('link[rel="canonical"]').attr('href');
    if (canonicalUrl && canonicalUrl.startsWith('https:')) {
      scores.httpsUsage = 5;
    } else {
