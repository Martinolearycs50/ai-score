...
function findUnexplainedTerms(text: string): string[] {
  ...
  for (const [term, explanation] of Object.entries(TECHNICAL_TERMS)) {
    ...
    if (termRegex.test(text)) {
      ...
      const explanationPattern = new RegExp(`${term}.*?[\\(\\[\\-–—].*?${explanation}|${explanation}.*?${term}`, 'is');
...
function isActionOriented(text: string): boolean {
  const actionVerbs = [
    'add', 'use', 'create', 'update', 'remove', 'enable', 'implement',
    'wrap', 'move', 'replace', 'convert', 'link', 'display', 'set',
    'include', 'offer', 'break up', 'restructure'
  ];
...
describe('Recommendation Clarity Tests', () => {
  ...
  describe('Language Clarity and Sophistication', () => {
    ...
    test.each(recommendations)('%s maintains professional tone without jargon overload', (key, rec) => {
  ...
  describe('Specific Recommendation Checks', () => {
    test('CDN recommendation explains what CDN means', () => {
      const cdnMentions = recommendations.filter(([k, r]) => 
        r.fix.includes('CDN') || r.why.includes('CDN')
      );
