...
import { type ScoringResult, score } from './scorer-new';
import type { PageType, PillarResults, WebsiteProfile } from './types';
...
export class AiSearchAnalyzer {
  ...
  async analyzeUrl(url: string): Promise<AnalysisResultNew> {
    ...
    try {
      ...
      try {
        ...
        console.log(
          `[AiSearchAnalyzer] Page type: ${extractedContent.pageType}, Primary topic: ${extractedContent.primaryTopic}`
        );
      ...
      const websiteProfile = this.buildWebsiteProfile(
        normalizedUrl,
        $,
        pageTitle,
        pageDescription,
        extractedContent?.pageType || 'general'
      );
      ...
      const pillarResults: PillarResults = {
        ...
        TRUST: await trust.run(html, normalizedUrl),
  ...
  private detectContentType(
    ...
    urlObj: URL,
    title: string
  ): WebsiteProfile['contentType'] {
    const path = urlObj.pathname.toLowerCase();
    const bodyText = $('body').text().toLowerCase();

    // E-commerce indicators
    if (
      $('.product, .price, .add-to-cart, .cart, .checkout').length > 0 ||
    ...
    // News indicators
    if (
      $('.article-date, .publish-date, .byline, .news-category').length > 0 ||
      path.includes('/news/') ||
    ...
    if (
      $('.docs, .documentation, .api-reference, .code-sample').length > 0 ||
      path.includes('/docs/') ||
    ...
    // Blog indicators
    if (
      $('.blog-post, .post-date, .author-bio, .comments').length > 0 ||
      path.includes('/blog/') ||
      path.includes('/post/')
    ) {
      return 'blog';
    ...
    if (
      $('.about-us, .services, .team, .contact-us').length > 0 ||
