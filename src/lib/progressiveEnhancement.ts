// Progressive enhancement for AI Search Score // Provides instant results via Cloudflare Worker,
then enhances with full API;
...
onError: (error: Error,
phase: 'quick' | 'full') => void;
}
 // Worker URL - will be replaced with actual deployed URL const WORKER_URL = process.env.NEXT_PUBLIC_WORKER_URL || 'https://ai-search-worker.your-subdomain.workers.dev';
...
console.error('Full analysis failed:',
error);
callbacks.onError(error as Error,
'full');
}
...
basicMetrics }
 = quickResult;
// Calculate total score const totalScore = Object.values(quickScores).reduce((sum,
score) => sum + score, 0);
  return {
