...
export class DynamicRecommendationGenerator {
   ...
   */
  generateRecommendation(metric: string, baseTemplate: RecommendationTemplate): RecommendationTemplate {
  ...
  private generateDirectAnswerExample(): { before: string; after: string } {
    ...
    const genericContent = this.content.contentSamples.paragraphs[0] || 'Content about the topic...';
  ...
  private createDirectAnswer(question: string, content: string): string {
    ...
    if (questionLower.includes('what is')) {
      const subject = question.replace(/what is/i, '').replace(/\?/g, '').trim();
      return `${subject} is ${content.substring(0, 100)}...`;
    }
    
    if (questionLower.includes('how')) {
      return `To ${question.replace(/how (to|do|does)/i, '').replace(/\?/g, '').trim()}, ${content.substring(0, 80)}...`;
    }
