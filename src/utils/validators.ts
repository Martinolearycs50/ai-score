...
export function validateAndNormalizeUrl(input: string): UrlValidationResult {
  ...
  try {
    ...
    try {
      ...
      console.error(
        `${logPrefix} URL constructor error message:`,
    ...
    if (!hostname.includes('.') || hostname.endsWith('.')) {
      console.log(
        `${logPrefix} Failed: invalid domain format (no TLD or ends with dot):`,
        hostname
      );
      return {
        isValid: false,
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith('user-agent:')) {
      ...
      if (disallowPath === '/' || disallowPath === '') {
        ...
        if (currentUserAgent === '*' || currentUserAgent.includes('perplexitybot')) {
          result.perplexity = false;
        }

        if (currentUserAgent === '*' || currentUserAgent.includes('googlebot')) {
          result.gemini = false;
        }
      }
    }
