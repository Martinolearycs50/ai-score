...
expect(result.avgSentenceLength).toBe(1);
  ...
  // Test boundary conditions for each level const levels = [ {
...
expectedLevel }) => {
const result = calculateReadingLevel(text);
// Allow for some flexibility in categorization due to approximations expect(['Very Easy',
'Easy',
'Fairly Easy',
'Standard',
'Fairly Difficult',
'Difficult',
'Very Difficult']) .toContain(result.level);
});
});
});
