...
class ContentAccuracyVerifier {
  ...
  private verifyScoreCalculations(result: AnalysisResultNew) {
    ...
    if (result.breakdown) {
      for (const [pillar, checks] of Object.entries(result.breakdown)) {
        const checkSum = Object.values(checks).reduce(
          (a: number, b: any) => a + (typeof b === 'number' ? b : 0),
          0
        );
        const pillarScore =
          result.scoringResult.pillarScores[
            pillar as keyof typeof result.scoringResult.pillarScores
          ];
  ...
  private addIssue(issue: VerificationIssue) {
    ...
    console.log(
      `${prefix} [Content Verification] ${issue.component}.${issue.field}: ${issue.message}`
    );
  ...
  generateReport(): string {
    ...
    // Group by component
    const byComponent = this.issues.reduce(
      (acc, issue) => {
    ...
    for (const [component, issues] of Object.entries(byComponent)) {
      ...
      issues.forEach((issue) => {
        ...
        report.push(
          `    Displayed: ${JSON.stringify(issue.displayed)} | Actual: ${JSON.stringify(issue.actual)}`
        );
      });
    }
...
// Export the verifier class
export const contentVerifier = new ContentAccuracyVerifier();
