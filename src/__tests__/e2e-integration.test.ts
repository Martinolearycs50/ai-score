...
test.describe('End-to-End API Integration', () => {
  ...
  test('Progressive enhancement: Quick results → Full analysis → CrUX enhancement', async ({ page }) => {
  ...
  test('Error handling: Graceful degradation when APIs fail', async ({ page }) => {
    ...
    const hasEnhancementError = errorLogs.some(log => 
      log.includes('Enhancement failed') || log.includes('enhancement')
    );
