// Simplified mock for cheerio that properly handles all test cases const cheerio = {
  load:  jest.fn((html: string) => {
    ...
    // Create a mock jQuery-like function const $ = jest.fn((selector: any) => {
      
      // Handle when selector is an element (from filter callback,
      etc) if (typeof selector !== 'string') {
        
        
        // Return appropriate content based on the element type if (selector.tagName === 'SCRIPT') {
 ...
 // Create mock results based on actual HTML content const results: any = {
      elements:  [],
      text:  jest.fn(() => {
 ...
 if (selector.includes('main') || selector.includes('article') || selector === 'body') {
          // Remove all HTML tags and return text content return html.replace(/<[^>]+>/g,
          ' ').replace(/\s+/g,
          ' ').trim();
        };
 return 'Mock text content';
      ...
      html: jest.fn(() => {
        if (selector.includes('script[type="application/ld+json"]')) {
          // Extract JSON content from the first matching script tag const scriptMatch = html.match(/<script[^>]*type="application\/ld\+json"[^>]*>([\s\S]*?)<\/script>/);
          if (scriptMatch) {
 ...
 // Handle both property and name meta tags if (selector.includes('property="')) {
 const property = selector.match(/property="([^"]+)"/)?.[1];
 if (property) {
 const regex = new RegExp(`<meta[^>]*property="${
property}
"[^>]*content="([^"]+)"`);
...
"[^>]*content="([^"]+)"`);
 ...
 return undefined;
        ...
        each: jest.fn((callback: (i: number,
        el: any) => void) => {
          if (selector.includes('h1, h2, h3, h4, h5, h6')) {
            // Extract all headings from HTML const headingRegex = /<(h[1-6])[^>]*>([^<]*)<\/\1>/gi;
            let match;
            let index = 0;
            while ((match = headingRegex.exec(html)) !== null) {
              ...
              // Extract paragraphs const paragraphRegex = /<p[^>]*>([^<]*)<\/p>/gi;
              let match;
              let index = 0;
                ...
                if (match[1].trim()) {
                  callback(index++, {
                    ...
                    while ((match = scriptRegex.exec(html)) !== null) {
                      ...
                      // Create a mock element that the structure module expects const mockScriptEl = {
 ...
 callback(index++, {
 getAttribute: (attr: string) => attr === 'href' ? match[1] : null, textContent: match[2] });
 }
 };
 ...
 getAttribute: (attr: string) => attr === 'href' ? match[1] : null, textContent: match[2] });
                      ...
                      // If we're filtering links, simulate the citation detection if (selector.includes('a[href^="http"]')) {
                        const linkRegex = /<a[^>]*href="(https?:\/\/[^"]+)"[^>]*>([^<]*)<\/a>/gi;
                        const filteredLinks = [];
                        let match;
                        let index = 0;
                        while ((match = linkRegex.exec(html)) !== null) {
                          ...
                          const href = match[1];
                          const text = match[2];
                          const context = `According to ${
text};
 research study findings show`;
                          // Check if this link would pass the citation filter const isCitation = href.includes('.gov') || href.includes('.edu') || href.includes('.org') || href.includes('doi.org') || href.includes('pubmed') || href.includes('arxiv') || href.includes('scholar') || context.toLowerCase().includes('source') || context.toLowerCase().includes('study') || context.toLowerCase().includes('research') || context.toLowerCase().includes('report') || text.includes('[');
 ...
 return {
                      ...
                      eq: jest.fn(() => results), // Set length based on what was found length: 0
                    };
                    // Update length based on selector if (selector === 'table') {
                      results.length = html.includes('<table')
                      ?
                      1
                      :
                      0;
                    ...
                    if
                    (selector.includes('ul, ol')) {
                      results.length = (html.includes('<ul')
                      ||
                      html.includes('<ol'))
                      ?
                      3
                    ...
                    if
                    (selector.includes('h1, h2, h3, h4, h5, h6')) {
 ...
 return results;
                    ...
                    // Add global methods to $ ($ as any).text = jest.fn(() => html.replace(/<[^>]+>/g,
                    ' ').replace(/\s+/g,
                    ' ').trim());
